...
def download_data_from_s3(ticker, S3_BUCKET_NAME, s3_client):
    # Try to get the stock data (CSV file) and metrics data (CSV file) from the S3 bucket
    try:
        # Construct the S3 keys (paths) for the stock data and metrics data CSV files
        stock_s3_key = f"first_strategy/{ticker}_strategy_data.csv"  # e.g., "AAPL_stock_data.csv"
        metrics_s3_key = f"first_strategy/{ticker}_metrics.csv"  # e.g., "AAPL_metrics_data.csv"

        # Download the stock data CSV from the S3 bucket
        stock_obj = s3_client.get_object(Bucket=S3_BUCKET_NAME, Key=stock_s3_key)
        stock_data = pd.read_csv(io.BytesIO(stock_obj['Body'].read()))

        # Download the metrics data CSV from the S3 bucket
        metrics_obj = s3_client.get_object(Bucket=S3_BUCKET_NAME, Key=metrics_s3_key)
        metrics_data = pd.read_csv(io.BytesIO(metrics_obj['Body'].read()))
